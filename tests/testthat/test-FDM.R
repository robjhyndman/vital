...
test_that("Functional data model", {
  if (requireNamespace("feasts", quietly = TRUE)) {
    ...
    expect_identical(colnames(glance(hu)), c("Sex", "State", "Code", ".model", "nobs", "varprop"))
    ...
    expect_identical(forecast(hu, bootstrap = TRUE, times = 7L) |>
      head(1) |>
      dplyr::pull(Mortality) |>
      unlist() |>
      length(), 7L)
    expect_identical(colnames(time_components(hu)), c("Sex", "State", "Code", "Year", "mean", paste0("beta", 1:6)))
    expect_identical(colnames(age_components(hu)), c("Sex", "State", "Code", "Age", "mean", paste0("phi", 1:6)))
  }
})
