...
test_that("classes", {
  ...
  expect_s3_class(
    aus_mortality |> arrange(Age, Sex, State, Code, Year),
    "vital"
  )
  ...
  expect_s3_class(
    bind_rows(
      aus_mortality |> filter(Year == 1901),
      aus_mortality |> filter(Year == 1902)
    ),
    "vital"
  )
  expect_s3_class(
    bind_cols(
      aus_mortality |> select(-Exposure),
      Exposure = aus_mortality$Exposure
    ),
    "vital"
  )
  ...
  expect_s3_class(
    aus_mortality |> group_by(Sex) |> summarise(Deaths = mean(Deaths)),
    "vital"
  )
