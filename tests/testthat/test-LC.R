...
test_that("Lee Carter", {
  ...
  expect_equal(
    dplyr::filter(
      ...
      dplyr::pull(.mean), 0.002446,
    tolerance = 1e-5
  )
  expect_identical(forecast(lc, bootstrap = TRUE, times = 7L) |>
    head(1) |>
    dplyr::pull(Mortality) |>
    unlist() |>
    length(), 7L)
  expect_identical(colnames(time_components(lc |> select(fit))), c("Sex", "State", "Code", "Year", "kt"))
