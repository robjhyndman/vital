...
#' Fetch data from the Human Mortality Database
#'
...
#' @param year_range,age_range A single integer indicating the period or
...
hmd_data <- function(country,
  ...
  res <- purrr::map(stats, function(astat) {
    switch(astat,
...
hmd_life_expectancy <- function(country, year_range = 1) {
  ...
  out <- purrr::map(seq_along(country), function(i) {
    read_hmd_file(
      country[i],
      filename,
      country_labels[i]
...
read_hmd_file <- function(country, filename, label = country) {
  ...
  if (is.null(.hmd$username) | is.null(.hmd$password)) {
    rlang::abort("Use `hmd_session()` to set the username and password")
  }
...
check_hmd_session <- function() {
  if (is.null(.hmd$username) | is.null(.hmd$username)) {
    hmd_session()
    if (is.null(.hmd$username) | is.null(.hmd$username)) {
      rlang::abort("You need to supply your HMD username or password with `hmd_session()`")
    }
  }
}
