...
```{r mortality, fig.cap="First two components of the functional data model for mortality rates."}
...
```{r fertility, fig.cap = "Fitted values of the functional mean model for fertility rates."}
...
```{r migration, fig.cap="First two components of the functional data model for net migration."}
...
```{r population_plot, fig.cap="Simulated population for the first replicate."}
future |>
  filter(.rep == "100") |>
...
```{r population_pyramid, fig.cap="Population pyramid for 2032 with 95% prediction intervals."}
pyramid_2032 <- future |>
  filter(Year == 2032) |>
