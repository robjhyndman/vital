...
get_aus_mortality <- function(code) {
  deaths <- read.table(
    paste0("https://aushd.org/assets/txtFiles/humanMortality/",code,"/Deaths_1x1.txt"),
    skip = 2, header = TRUE
  ) |>
    as_tibble() |>
    ...
    mutate(Code = code)
 read.table(
    paste0("https://aushd.org/assets/txtFiles/humanMortality/",code,"/Exposures_1x1.txt"),
    skip = 2, header = TRUE
  ) |>
    as_tibble() |>
    tidyr::pivot_longer(Female:Total, names_to = "Sex", values_to = "Exposure") |>
    left_join(deaths) |>
    mutate(
    ...
    as_vital(index = Year, key = c(Age, Sex, Code),
             .age = "Age", .sex = "Sex", .deaths = "Deaths", .population = "Exposure") |>
   collapse_ages()
}
