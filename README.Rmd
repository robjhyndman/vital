...
This example contains data from `r min(nor$Year)` to `r max(nor$Year)`. It must have a time index variable (here `Year`), and optionally other categorical variables known as "key" variables. Each row must have a unique combination of the index and key variables. Some columns are "vital" variables, such as "Age" and "Sex".
